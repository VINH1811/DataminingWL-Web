/* =========================================================
   WL-Datamining Lab 1 JS - Enhanced Van Gogh Premium
   - Starry canvas animation with swirls
...
(() => {
  ...
  if (starryCanvas) {
    ...
    // Star particles with swirl motion
    class Star {
      constructor() {
        this.reset();
      }
      ...
      update() {
        // Swirl motion (Van Gogh style)
        this.angle += this.orbitSpeed;
        const swirl = Math.sin(this.angle) * 0.5;
        
        this.x += this.speedX + swirl;
        this.y += this.speedY;
      ...
      draw() {
        ...
        gradient.addColorStop(0, `rgba(159, 215, 249, ${this.opacity * 0.4})`);
        gradient.addColorStop(1, "rgba(159, 215, 249, 0)");
    ...
    // Animation loop
    const animateStars = () => {
      ctx.fillStyle = "rgba(6, 21, 37, 0.1)";
  ...
  const setActiveByPath = () => {
    const path = (location.pathname.split("/").pop() || "lab1.html").toLowerCase();
    const mark = (a) => {
  ...
  /* ========== ROPE TIMELINE TRACKER ========== */
/* ========== ROPE TIMELINE TRACKER - SCROLL-BASED (ULTRA SMOOTH) ========== */
const panels = $$(".panel");
...
if (ropeDots && panels.length) {
  ...
  const updateRope = () => {
    ...
    // Tìm panel gần nhất với center viewport
    let closestIndex = 0;
    ...
    // CHỈ update khi thay đổi (tránh re-render)
  ...
  // Hero buttons -> open Lab1 PDF
  const openLabPdf = $("#openLabPdf");
  const openLabPdf2 = $("#openLabPdf2");
  
  const pdfConfig = {
    pdf: "pdf/blog1.pdf",
    title: "Lab 1 • Product Hub Analysis",
    repo: "https://github.com/VINH1811/shopping_cart_analysis",
  };
